...
- `src/stack.py` - `STACK: List[Tuple[window_id, group, sheet_ids, focused_id]]`; serialization only. Medium + throttle live in `src/cache.py` (`read_cache`/`write_cache`, per-key budgets, `stack_cache_throttle` setting min 30, default 30); key `compass_stack_cache`, tuple format frozen.
...
- `src/commands/broadcast.py` - no-op `compass_broadcast_event` window command + `dispatch_event(window, event, **payload)` helper (in `src/plugins_registry.py`): the core→plugin transport. Every event runs the command (any outside package can snoop via `on_window_command` with zero imports, JSON payload with ids) and `dispatch_event` calls `on_<event>` directly on every enabled registered plugin with live objects. Payloads: `highlight`/`select` (`item`, `meta`; broadcast carries `trigger`), `folders_changed`, `sheet_activated`/`sheet_closed` (`sheet`), `window_closed`, `project_closed`, `project_loaded`. Compass dispatches `highlight`/`select` from `show.py` to every plugin (ownership guard via `is_applicable` inside each handler; `_is_claimed` gates the SheetGroup fallback) and the rest from `check_folders_changed()` / `CompassFocusListener` (`src/events.py`). No folder-add listener from Sublime itself (GH #4753, #2234): detection is a cheap core-owned folder-list snapshot, not ripgrep walks or timer polls — do not re-add folder diffing without owner approval.
